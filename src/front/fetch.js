...
if (!API_URL) {
  console.warn(
    "VITE_BACKEND_URL not set. Using default: http://localhost:3000"
  );
}
...
const getAuthHeaders = () => {
  ...
  if (!token) {
    console.warn(
      "No token found in sessionStorage - user may not be authenticated"
    );
  }
...
export const providerAPI = {
  ...
  updateService: async (serviceId, serviceData) => {
    const response = await fetch(
      `${API_URL}/api/provider/services/${serviceId}`,
      {
        method: "PUT",
        headers: getAuthHeaders(),
        body: JSON.stringify(serviceData),
  ...
  getBookingDetails: async (bookingId) => {
    const response = await fetch(
      `${API_URL}/api/provider/bookings/${bookingId}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  updateBookingStatus: async (bookingId, status) => {
    const response = await fetch(
      `${API_URL}/api/provider/bookings/${bookingId}/status`,
      {
        method: "PUT",
        headers: getAuthHeaders(),
  ...
  getAllMessages: async () => {
    const response = await fetch(`${API_URL}/api/provider/messages`, {
      headers: getAuthHeaders(),
    });
  ...
  sendMessage: async (customerId, message) => {
    const response = await fetch(`${API_URL}/api/messages/provider/send`, {
      method: "POST",
      headers: getAuthHeaders(),
      body: JSON.stringify({
  ...
  deleteConversation: async (customerId) => {
    const response = await fetch(`${API_URL}/api/provider/conversations/${customerId}`, {
      method: "DELETE",
      headers: getAuthHeaders(),
    });
...
export const customerAPI = {
  ...
  sendMessage: async (providerId, message, serviceId) => {
  ...
  getMessages: async (providerId) => {
    const response = await fetch(`${API_URL}/api/messages/${providerId}`, {
      headers: getAuthHeaders(),
    });
    if (!response.ok) throw new Error("Failed to fetch messages");
    return response.json();
  ...
  getRecentBookings: async () => {
    const response = await fetch(`${API_URL}/api/customer/recent-bookings`, {
      headers: getAuthHeaders(),
    });
  ...
  deleteConversation: async (providerId) => {
    const response = await fetch(`${API_URL}/api/conversations/${providerId}`, {
      method: "DELETE",
      headers: getAuthHeaders(),
    });
    if (!response.ok) throw new Error("Failed to delete conversation");
